...
use XWC\Decorators\Data_Type_Extension;
...
final class Data_Type_Manager {
    ...
    public function register_class( string $classname, string ...$deps ): Data_Type|\WP_Error {
        ...
        try {
            ...
            $ext = $this->get_extensions( $classname );
			$def = $this->get_definition( $classname );
            $dt  = $def->name;
            ...
            $dto = new Data_Type( $classname, $def, $deps );
     ...
     */
    protected function get_definition( string $classname ): Data_Type_Definition {
        $definition = null;
        $classes    = $this->get_class_parents( $classname );

        foreach ( $classes as $class ) {
            $definition = $this->get_decorator( $class, Data_Type_Definition::class );
    ...
    /**
     * Get the data type extensions.
     *
     * @param  string $classname Data type class name.
     * @return array<int, Data_Type_Extension>
     */
    protected function get_extensions( string $classname ): array {
        $ext = array();

        foreach ( $this->get_class_parents( $classname, true ) as $class ) {
            $ext[] = $this->get_decorator( $class, Data_Type_Extension::class );
        }
        return \array_map(
            static fn( $e ) => $e->newInstance(),
            \array_filter( $ext ),
        );
    }
